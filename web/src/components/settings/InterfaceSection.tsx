...
const DEFAULTS: InterfacePrefs = { density: "expanded", reduceMotion: false };

function load(): InterfacePrefs {
  if (typeof window === "undefined") return DEFAULTS;
  try {
    const raw = window.localStorage.getItem(KEY);
    if (!raw) return DEFAULTS;
    return { ...DEFAULTS, ...(JSON.parse(raw) as Partial<InterfacePrefs>) };
  } catch {
    return DEFAULTS;
  }
}

function save(p: InterfacePrefs) {
  try {
...
export function InterfaceSection() {
  ...
  const [p, setP] = useState<InterfacePrefs>(DEFAULTS);
  ...
  function update(patch: Partial<InterfacePrefs>) {
    const next = { ...p, ...patch };
    setP(next);
    save(next);
