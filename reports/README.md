...
- 005-backend-architecture-review — Read-only backend audit (data model/RLS, AI core, API surface, frontend/testing) + executed typecheck/test/lint for real build-health evidence. No code changed; findings feed the fix-first list for the next session. (Filed as a separate PR; may need a trivial merge-order fixup with 006/007 below.)
...
- 007-memory-layer-architecture-review — Read-only audit of the memory subsystem (data model, `lib/ai-brain/*` synthesizer pipeline, memory API contract, graph UI). No code changed; companion to 005/006. Also filed as a separate PR — same merge-order caveat as above.
